...
function awardXP(db, username, action, customDesc, req) {
  ...
  try {
    ...
    // Check for rank up
    const oldXp = getUserXP(db, username) - cfg.xp;
    const newXp = getUserXP(db, username);
    const oldRank = getRank(oldXp);
    const newRank = getRank(newXp);
    const rankUp = newRank.name !== oldRank.name ? newRank.icon + ' ' + newRank.name : null;
    // Set flash in session if available
    if (req && req.session) {
      req.session.xpFlash = { xp: cfg.xp, label: customDesc || cfg.label, rankUp };
    }
    return cfg.xp;
  } catch(e) { return null; }
}
